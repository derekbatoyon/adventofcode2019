    ...
    while len(vacuum) > 0:
        ...
        for location in locations:
            for direction in all_directions:
                loc = movement[direction](location)
                if loc in vacuum:
                    vacuum.remove(loc)
                    edge.append(loc)
        t += 1
        draw_map(map, [l for l in map.iterkeys() if l not in vacuum])
        time.sleep(0.25)
